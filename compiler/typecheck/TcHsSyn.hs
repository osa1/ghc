...
topSigWarnId sig_ns id
  | idName id `elemNameSet` sig_ns
  = warnMissingSig msg Opt_WarnMissingExportedSigs id
  | otherwise
  = return ()
  where
...
localSigWarnId sig_ns id
  | not (isSigmaTy (idType id))
  = return ()
  | idName id `elemNameSet` sig_ns
  = return ()
  | otherwise
  = warnMissingSig msg Opt_WarnMissingLocalSigs id
  where
    msg = ptext (sLit "Polymorphic local binding with no type signature:")

warnMissingSig :: SDoc -> WarningFlag -> Id -> TcM ()
warnMissingSig msg flag id
  = do  { env0 <- tcInitTidyEnv
        ; let (env1, tidy_ty) = tidyOpenType env0 (idType id)
        ; addWarnTcM env1 (mk_msg tidy_ty) (Just flag) }
  where
