...
-- _before_ it's actually evaluated, by floating dataToTag# out), and check
-- evaluted-ness of its argument in CorePrep and introduce a case if the argument
-- is not already evaluated. However, it was discovered in #15696 that we can't
-- really use evaluted-ness of the argument (as seen in Core) to avoid entering the
-- argument before reading the tag. So we now lower dataToTag# in StgCmmExpr and
-- ensure that we always enter the argument.
--
-- dataToTag# is still marked as can_fail because we still don't want floating it
-- out, as that'd cause evaluating the argument more eagerly. See also Note [PrimOp
-- can_fail and has_side_effects] in PrimOp.
--
